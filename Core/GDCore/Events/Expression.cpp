...
namespace gd {
  ...
  Expression::Expression(gd::String plainString_) : node(nullptr), plainString(plainString_){};

  Expression::Expression(const char* plainString_) : node(nullptr), plainString(plainString_){};
...
std::unique_ptr<ExpressionNode> Expression::GetRootNode(const gd::String &type, gd::ExpressionParser2 &parser) const {
  if (!node) {
