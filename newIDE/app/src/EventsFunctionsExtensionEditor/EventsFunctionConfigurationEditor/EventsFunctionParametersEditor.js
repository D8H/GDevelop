...
export const EventsFunctionParametersEditor = ({
  ...
  const copyParameter = React.useCallback(
    (parameter: gdNamedParameterDescriptor) => {
  ...
  const pasteParameters = React.useCallback(
    async propertyInsertionIndex => {
      ...
      propertyContents.forEach(propertyContent => {
        const name = SafeExtractor.extractStringParameter(
          propertyContent,
          'name'
        );
        const serializedParameter = SafeExtractor.extractObjectParameter(
          propertyContent,
          'serializedParameter'
        );
      ...
      newNamedParameters.forEach(({ name, serializedParameter }) => {
        const parameter = parameters.insertNew(name, index);
        index++;
  ...
  const pasteParametersAtTheEnd = React.useCallback(
    async () => {
      await pasteParameters(eventsFunction.getParameters().getCount());
    },
    [eventsFunction, pasteParameters]
  );

  const pasteParametersBefore = React.useCallback(
    async (parameter: gdNamedParameterDescriptor) => {
      await pasteParameters(
        eventsFunction.getParameters().getPosition(parameter)
      );
    },
    [eventsFunction, pasteParameters]
  );
  ...
  const moveParameterBefore = React.useCallback(
    (targetParameter: gdNamedParameterDescriptor) => {
      const { current } = draggedParameter;
      if (!current) return;

      const parameters = eventsFunction.getParameters();
      const draggedIndex = parameters.getPosition(current);
      const targetIndex = parameters.getPosition(targetParameter);
