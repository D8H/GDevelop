...
function ObjectsList({
  ...
  const [tagEditedObject, setTagEditedObject] = React.useState<?gdObject>(null);
  ...
  const onObjectModified = React.useCallback(
    ...
    [forceUpdateList, unsavedChanges]
  );
  ...
  const openEditTagDialog = React.useCallback((tagEditedObject: ?gdObject) => {
  ...
  const renderObjectMenuTemplate = React.useCallback(
    (i18n: I18nType) => (
      ...
      return [
        ...
        onExportObject && project.hasEventsBasedObject(object.getType())
          ? {
